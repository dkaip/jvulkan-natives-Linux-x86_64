 ...
 */
/*
 * vkGetValidationCacheDataEXT.cpp
 *
...
JNIEXPORT jobject JNICALL Java_com_CIMthetics_jvulkan_VulkanCore_VK11_NativeProxies_vkGetValidationCacheDataEXT
  (JNIEnv *env, jobject, jobject jVkDevice, jobject jVkValidationCacheEXT, jbyteArray jByteArrayReturnValue)
{
	...
	VkValidationCacheEXT_T *validationCacheHandle = (VkValidationCacheEXT_T *)jvulkan::getHandleValue(env, jVkValidationCacheEXT);
	if (env->ExceptionOccurred())
	{
	...
	VkResult result = vkGetValidationCacheDataEXT(
			deviceHandle,
	...
	{
		LOGERROR(env, "vkGetValidationCacheDataEXT returned %d.", result);
