  ...
  (JNIEnv *env, jobject, jstring jLayerName, jobject jVkExtensionPropertiesCollection)
{
    ...
    if (propertyCount != 0)
    {
		...
		jclass vkExtensionPropertiesClass = env->FindClass("com/CIMthetics/jvulkan/VulkanCore/Structures/VkExtensionProperties");
        if (env->ExceptionOccurred())
        {
        	LOGERROR(env, "%s", "Could not find class com/CIMthetics/jvulkan/VulkanCore/Structures/VkExtensionProperties");
            if (jLayerName != nullptr)
            {
                env->ReleaseStringUTFChars(jLayerName, layerName);
                if (env->ExceptionOccurred())
