 ...
 */
/*
 * vkEnumerateInstanceLayerProperties.cpp
 *
...
JNIEXPORT jobject JNICALL Java_com_CIMthetics_jvulkan_VulkanCore_VK11_NativeProxies_vkEnumerateInstanceLayerProperties
  (JNIEnv *env, jobject, jobject jVkLayerPropertiesCollection)
{
    ...
    {
    	LOGERROR(env, "%s", "Call to vkEnumerateInstanceLayerProperties failed.");
    ...
    if (propertyCount != 0)
    {
		...
		jclass vkLayerPropertiesClass = env->FindClass("com/CIMthetics/jvulkan/VulkanCore/VK11/Structures/VkLayerProperties");
        if (env->ExceptionOccurred())
        {
        	LOGERROR(env, "%s", "Could not find class com/CIMthetics/jvulkan/VulkanCore/VK11/Structures/VkLayerProperties");
