 ...
 */
/*
 * populateVkDisplayNativeHdrSurfaceCapabilitiesAMD.cpp
 *
...
namespace jvulkan
{
			...
			std::vector<void *> *memoryToFree)
	{
        ...
        ///////////////////////////////////////////////////////////////////////////
        jmethodID methodId = env->GetMethodID(theClass, "setLocalDimmingSupport", "(Z)V");
        if (env->ExceptionOccurred())
        {
        	LOGERROR(env, "%s", "Could not find method id setLocalDimmingSupport");
            return;
        }

        env->CallVoidMethod(jVkDisplayNativeHdrSurfaceCapabilitiesAMDObject, methodId, vkDisplayNativeHdrSurfaceCapabilitiesAMD->localDimmingSupport);
        if (env->ExceptionOccurred())
        {
