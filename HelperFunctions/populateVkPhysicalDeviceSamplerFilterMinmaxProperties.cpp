 ...
 */
/*
 * populateVkPhysicalDeviceSamplerFilterMinmaxProperties.cpp
 *
...
namespace jvulkan
{
			...
			std::vector<void *> *memoryToFree)
	{
		...
		jclass theClass = env->GetObjectClass(
				jVkPhysicalDeviceSamplerFilterMinmaxPropertiesObject);
		if (env->ExceptionOccurred())
		{
			LOGERROR(env, "%s", "Could not find class for jVkPhysicalDeviceSamplerFilterMinmaxPropertiesObject.");
			return;
		}
        ...
        env->CallVoidMethod(jVkPhysicalDeviceSamplerFilterMinmaxPropertiesObject, methodId, vkPhysicalDeviceSamplerFilterMinmaxProperties->filterMinmaxSingleComponentFormats);
        if (env->ExceptionOccurred())
        {
        ...
        env->CallVoidMethod(jVkPhysicalDeviceSamplerFilterMinmaxPropertiesObject, methodId, vkPhysicalDeviceSamplerFilterMinmaxProperties->filterMinmaxImageComponentMapping);
        if (env->ExceptionOccurred())
        {
        	LOGERROR(env, "%s", voidMethodErrorText);
            return;
