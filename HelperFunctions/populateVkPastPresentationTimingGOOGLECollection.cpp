...
namespace jvulkan
{
			...
			int numberOfVkPastPresentationTimingGOOGLEs)
	{
        ...
        jclass vkPastPresentationTimingGOOGLEClass = env->FindClass("Lcom/CIMthetics/jvulkan/VulkanExtensions/VK11/Structures/VkPastPresentationTimingGOOGLE;");
        if (env->ExceptionOccurred())
        {
        	LOGERROR(env, "%s", "Could not find class com/CIMthetics/jvulkan/VulkanExtensions/VK11/Structures/VkPastPresentationTimingGOOGLE");
            return;
        }

        jmethodID constructorMethodId = env->GetMethodID(vkPastPresentationTimingGOOGLEClass, "<init>", "()V");
        if (env->ExceptionOccurred())
        {
