 ...
 */
/*
 * populateVkSurfaceProtectedCapabilitiesKHR.cpp
 *
...
namespace jvulkan
{
			...
			std::vector<void *> *memoryToFree)
	{
        ...
        ///////////////////////////////////////////////////////////////////////////
        jmethodID methodId = env->GetMethodID(theClass, "setSupportsProtected", "(Z)V");
        if (env->ExceptionOccurred())
        {
        	LOGERROR(env, "%s", "Could not find method id setSupportsProtected");
            return;
        }

        env->CallVoidMethod(jVkSurfaceProtectedCapabilitiesKHRObject, methodId, vkSurfaceProtectedCapabilitiesKHR->supportsProtected);
        if (env->ExceptionOccurred())
        {
