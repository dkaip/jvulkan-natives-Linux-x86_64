 ...
 */
/*
 * createVkExternalSemaphoreFeatureFlagsAsEnumSet.cpp
 *
...
namespace jvulkan
{
    ...
    {
        char const *enumClassString = "com/CIMthetics/jvulkan/VulkanCore/VK11/Enums/VkExternalSemaphoreFeatureFlagBits";
        char const *enumObjectString = "Lcom/CIMthetics/jvulkan/VulkanCore/VK11/Enums/VkExternalSemaphoreFeatureFlagBits;";
         ...
         */
        if ((vkExternalSemaphoreFeatureFlags &
             !(VK_EXTERNAL_SEMAPHORE_FEATURE_EXPORTABLE_BIT |
			   VK_EXTERNAL_SEMAPHORE_FEATURE_IMPORTABLE_BIT)) != 0)
        {
        	LOGERROR(env, "Unhandled case for vkExternalSemaphoreFeatureFlags...value is %x", vkExternalSemaphoreFeatureFlags);
            return nullptr;
        }

        if (vkExternalSemaphoreFeatureFlags & VK_EXTERNAL_SEMAPHORE_FEATURE_EXPORTABLE_BIT)
        {
            jfieldID fieldId = env->GetStaticFieldID(enumClass, "VK_EXTERNAL_SEMAPHORE_FEATURE_EXPORTABLE_BIT", enumObjectString);
            ...
            if (addResult == false)
            {
            	LOGERROR(env, "%s", "Could not add VK_EXTERNAL_SEMAPHORE_FEATURE_EXPORTABLE_BIT to EnumSet");
                return nullptr;
            }
        }

        if (vkExternalSemaphoreFeatureFlags & VK_EXTERNAL_SEMAPHORE_FEATURE_IMPORTABLE_BIT)
        {
            jfieldID fieldId = env->GetStaticFieldID(enumClass, "VK_EXTERNAL_SEMAPHORE_FEATURE_IMPORTABLE_BIT", enumObjectString);
            jobject theEnum = env->GetStaticObjectField(enumClass, fieldId);

            bool addResult = env->CallBooleanMethod(enumSetObject, setAddMethod, theEnum);
            if (addResult == false)
            {
            	LOGERROR(env, "%s", "Could not add VK_EXTERNAL_SEMAPHORE_FEATURE_IMPORTABLE_BIT to EnumSet");
                return nullptr;
            }
        }
